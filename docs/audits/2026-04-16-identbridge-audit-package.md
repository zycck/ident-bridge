...
| `rg -n '^def test_' tests \| wc -l` | `157` test functions |
...
- The documented test gate in [docs/TESTING.md](/mnt/d/ProjectLocal/identa report/docs/TESTING.md:1) now matches the current tree, and the automated gate has since been reproduced on Windows 11 with Python 3.14.4 (`158 passed`) plus a clean `python main.py` smoke-run.
...
- Fresh Windows validation evidence after these changes:
  - `python -m pytest tests/ -q` → `158 passed in 2.03s`
...
| [app/ui/settings_widget.py](/mnt/d/ProjectLocal/identa report/app/ui/settings_widget.py:1) | `medium` | Smaller after SQL controller extraction, but autosave, startup, update flow, and settings UI state are still mixed together. |
