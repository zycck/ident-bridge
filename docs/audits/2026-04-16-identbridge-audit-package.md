...
| `rg -n '^def test_' tests \| wc -l` | `166` test functions |
...
- The documented test gate in [docs/TESTING.md](/mnt/d/ProjectLocal/identa report/docs/TESTING.md:1) now matches the current tree, and the automated gate has since been reproduced on Windows 11 with Python 3.14.4 (`167 passed`) plus a clean `python main.py` smoke-run.
...
- Fresh Windows validation evidence after these changes:
  - `python -m pytest tests/ -q` → `167 passed in 2.07s`
...
| [app/ui/main_window.py](/mnt/d/ProjectLocal/identa report/app/ui/main_window.py:1) | `medium` | Smaller after lifecycle extraction, but navigation, page wiring, debug panel management, and top-level shell orchestration are still mixed together. |
