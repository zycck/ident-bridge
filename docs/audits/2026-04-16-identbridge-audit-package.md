...
| `rg -n '^def test_' tests \| wc -l` | `144` test functions |
...
- The documented test gate in [docs/TESTING.md](/mnt/d/ProjectLocal/identa report/docs/TESTING.md:1) now matches the current tree, and the automated gate has since been reproduced on Windows 11 with Python 3.14.4 (`145 passed`) plus a clean `python main.py` smoke-run.
...
- Fresh Windows validation evidence after these changes:
  - `python -m pytest tests/ -q` → `145 passed in 1.94s`
