...
| `rg -n '^def test_' tests \| wc -l` | `271` test functions |
...
- The documented test gate in [docs/TESTING.md](/mnt/d/ProjectLocal/identa report/docs/TESTING.md:1) now matches the current tree, and the automated gate has since been reproduced on Windows 11 with Python 3.14.4 (`272 passed`). `python main.py` still starts correctly there, but shell-driven close verification currently follows the existing close-to-tray path and requires a force-stop for automated cleanup.
...
- Follow-up implementation waves after this audit have already reduced some of the highest-risk areas without changing user-facing behavior:
  ...
  - [app/ui/export_sql.py](/mnt/d/ProjectLocal/identa report/app/ui/export_sql.py:1) now lazy-loads `sqlglot`, and [app/ui/export_editor_controller.py](/mnt/d/ProjectLocal/identa report/app/ui/export_editor_controller.py:1) no longer triggers eager SQL validation on initial editor load, reducing retained allocations in the `export-editor` perf scenario.
...
- Fresh Windows validation evidence after these changes:
  - `python -m pytest tests/ -q` → `272 passed in 3.14s`
  ...
  - `python tools/perf_smoke.py --scenario export-editor --cycles 5 --top 8` → `positive_retained_kib=997.4`, down from the earlier `1681.1 KiB` after lazy-loading `sqlglot` and removing eager syntax validation on initial editor load
