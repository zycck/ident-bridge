...
Audit note: the current tree contains 272 `test_*` functions, and this
...
The fastest sanity check. **272 test functions / 273 collected test items** covering the scheduler engine,
export worker pipeline, config persistence, threading helpers, tray
behaviour, and Windows autostart.

The current tree actually contains **272 tests**. Keep this number in
...
- The repository tree currently reports 272 test functions, but the
