...
The fastest sanity check. **217 test functions / 218 collected test items** covering the scheduler engine,
export worker pipeline, config persistence, threading helpers, tray
behaviour, and Windows autostart.

The current tree actually contains **217 tests**. Keep this number in
...
- The repository tree currently reports 217 test functions, but the
