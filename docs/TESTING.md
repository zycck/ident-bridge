...
The fastest sanity check. **258 test functions / 259 collected test items** covering the scheduler engine,
export worker pipeline, config persistence, threading helpers, tray
behaviour, and Windows autostart.

The current tree actually contains **258 tests**. Keep this number in
...
- The repository tree currently reports 258 test functions, but the
