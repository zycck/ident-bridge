...
Audit note: the current tree contains 124 `test_*` functions, and this
...
The fastest sanity check. **124 test functions / 125 collected test items** covering the scheduler engine,
export worker pipeline, config persistence, threading helpers, tray
behaviour, and Windows autostart.

The current tree actually contains **124 tests**. Keep this number in
...
```
Core startup
  [ ] python -m pytest tests/ → 125/125 PASS (zero failures, zero errors)
...
- The repository tree currently reports 124 test functions, but the
