...
The fastest sanity check. **219 test functions / 220 collected test items** covering the scheduler engine,
export worker pipeline, config persistence, threading helpers, tray
behaviour, and Windows autostart.

The current tree actually contains **219 tests**. Keep this number in
...
- The repository tree currently reports 219 test functions, but the
