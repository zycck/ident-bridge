...
Audit note: the current tree contains 127 `test_*` functions, and this
...
The fastest sanity check. **127 test functions / 128 collected test items** covering the scheduler engine,
export worker pipeline, config persistence, threading helpers, tray
behaviour, and Windows autostart.

The current tree actually contains **127 tests**. Keep this number in
...
```
Core startup
  [ ] python -m pytest tests/ → 128/128 PASS (zero failures, zero errors)
...
- The repository tree currently reports 127 test functions, but the
