...
Audit note: the current tree contains 157 `test_*` functions, and this
...
The fastest sanity check. **157 test functions / 158 collected test items** covering the scheduler engine,
export worker pipeline, config persistence, threading helpers, tray
behaviour, and Windows autostart.

The current tree actually contains **157 tests**. Keep this number in
...
- The repository tree currently reports 157 test functions, but the
