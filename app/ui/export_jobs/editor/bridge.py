...
class ExportJobEditorBridge:
    ...
    def build_job(self) -> ExportJob:
        return ExportJob(
            ...
            gas_options={
                ...
                "write_mode": self._shell.gas_write_mode(),
            },
    ...
    def record_history_entry(self, entry: ExportHistoryEntry) -> None:
        if self._run_store is not None:
            self._run_store.record_history_entry(
                job_id=self._job_id,
                job_name=self._shell.job_name(),
                entry=entry,
            )
            return
        prepend = getattr(self._shell, "prepend_history_entry", None)
        if callable(prepend):
            prepend(entry)
