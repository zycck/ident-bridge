...
from app.ui.widgets import HeaderLabel, apply_light_window_palette, hsep
...
class ExportEditorShell(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        self.setStyleSheet(
            f"QWidget#exportCard {{ background-color: {Theme.surface_tinted}; }}"
        )
    ...
    def set_unfinished_runs(self, runs: list[ExportRunInfo]) -> None:
        self._history_panel.set_unfinished_runs(runs)
    ...
    def set_history_delete_handler(self, handler) -> None:
        self._history_panel.set_delete_handler(handler)

    def set_history_clear_handler(self, handler) -> None:
        self._history_panel.set_clear_handler(handler)

    def set_unfinished_retry_handler(self, handler) -> None:
        self._history_panel.set_unfinished_retry_handler(handler)

    def set_unfinished_reset_handler(self, handler) -> None:
        self._history_panel.set_unfinished_reset_handler(handler)

    def set_unfinished_delete_handler(self, handler) -> None:
        self._history_panel.set_unfinished_delete_handler(handler)
