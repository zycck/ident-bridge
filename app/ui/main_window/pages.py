...
def build_main_window_pages(
    ...
    run_store.migrate_legacy_history(config)
    stack = QStackedWidget()
    dashboard = DashboardWidget(config, run_store=run_store, parent=parent)
    export_jobs = ExportJobsWidget(config, run_store=run_store, parent=parent)
